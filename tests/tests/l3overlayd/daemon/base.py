...
# tests/l3overlayd/daemon/base.py - base class for daemon-level tests
#
...
class DaemonBaseTest(object):
    class Class(BaseTest.Class):
        ...
        def object_get(self, conf=None):
            ...
            d = daemon.read(conf if conf else self.global_conf)
            self.assertIsInstance(d, daemon.Daemon)
