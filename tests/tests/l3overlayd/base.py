...
# tests/l3overlayd/base.py - unit test base class
#
...
class BaseTest(object):
    ...
    Arguments: %s''' % (str.join(", ", (e.__name__ for e in exceptions)), object_conf))
        ...
        def assert_boolean(self, *args, test_default=False,
            ...
            self.assert_success(*args,
                value=True,
                expected_value=True, internal_key=internal_key,
                conf=conf
            )
            self.assert_success(*args,
                value="true",
            ...
            self.assert_success(*args,
                value=False,  
                expected_value=False, internal_key=internal_key,
                conf=conf
            )
            self.assert_success(*args,
                value="false",
        ...
        def assert_hex_string(self, *args, min=None, max=None, test_default=False,
            ...
            self.assert_fail(
                *args,
                value=str.join("", ["z" for __ in range(0, _min)]), exception=util.GetError,
                conf=conf
            )
        ...
        def assert_enum(self, *args, enum=None, test_default=False,
            ...
            for e in enum:
                self.assert_success(*args,
                    value=e.upper(), expected_value=e,
                    internal_key=internal_key,
                    conf=conf
                )
                self.assert_success(*args,
                    value=e.lower(), expected_value=e,
                    internal_key=internal_key,
                    conf=conf
                )
        ...
        def assert_path(self, *args,
            ...
            if assert_relative:
                ...
                self.assert_success(*args,
                    value=path,
                    expected_value=os.path.abspath(path),
                    internal_key=internal_key,
                    conf=conf
                )
